...
class AssetPrinter {
	...
	// 			}
		...
		if(true){
		//if(!$slideshowId->isEqual("0"){
     // if ($authZ->isUserAuthorized($idManager->getId(AZ_EDIT), $asset->getId())) {
	      		if ($actionString != "asset.add-to-slideshow") {
		      		$links[] = "<a href='".MYURL."/'>";
              $repositoryManager =& Services::getService("Repository");
              $idManager =& Services::getService("Id");
              $repositoryId =& $idManager->getId($harmoni->pathInfoParts[2]);
              $repository =& $repositoryManager->getRepository($repositoryId);
              $repository->_copyAsset($asset, $slideshowId);
           
				      $links[count($links) - 1] .= _("add to slideshow")."</a>";
			      } else {
				      $links[] = _("add-to-slideshow");
			      }
		 // }
    }
	...
	function printAssetFunctionLinksExh (& $harmoni, & $asset, $repositoryId = NULL) {
		if (!defined("AZ_ACCESS"))

...
if (!$authZ->isUserAuthorized($idManager->getId(AZ_EDIT), $id)) {
	...
	$errorLayout->addComponent(new Content(_("You are not authorized to edit this <em>Exhibition</em>."), MIDDLE, CENTER));
...
// Create the wizard.
 if ($_SESSION['edit_exhibition_wizard_'.$harmoni->pathInfoParts[2]]) {
 	$wizard =& $_SESSION['edit_exhibition_wizard_'.$harmoni->pathInfoParts[2]];
 } else {
	...
	$wizard =& new Wizard(_("Edit an Exhibition"));
	$_SESSION['edit_exhibition_wizard_'.$harmoni->pathInfoParts[2]] =& $wizard;
...
// 		printpre($properties);
	...
	// Save the Schema settings.
		...
		// Unset the wizard
		$wizard = NULL;
		unset ($_SESSION['edit_exhibition_wizard_'.$id->getIdString()]);
		unset ($wizard);

		// Head off to editing our new exhibition.
		$id =& $repository->getId();
		if ($_REQUEST['create_schema'])
			header("Location: ".MYURL."/schema/create/".$id->getIdString()."/".implode("/",$harmoni->pathInfoParts));
		else
			header("Location: ".MYURL."/exhibitions/namebrowse/");
	}

// Handle canceling if requested
} else if ($wizard->isCancelRequested()) {
	$wizard = NULL;
	unset ($_SESSION['edit_exhibition_wizard_'.$harmoni->pathInfoParts[2]]);
	unset ($wizard);
	header("Location: ".MYURL."/exhibitions/namebrowse/");

...
if (!isset($_SESSION['table_setup_complete'])) {
	...
	if (count ($tables)) {
		...
		// Course Management
		if (!in_array('cm_term', $tables))
			SQLUtils::runSQLfile(HARMONI_BASE."/SQL/CourseManagement/MySQL_CourseManagement.sql", $dbID);
			
		// Scheduling
		if (!in_array('sc_item_stat_type', $tables))
			SQLUtils::runSQLfile(HARMONI_BASE."/SQL/Scheduling/MySQL_Scheduling.sql", $dbID);
		
		// GUI
		if (!in_array('tm_style_collection', $tables))
			SQLUtils::runSQLfile(HARMONI_BASE."/SQL/GUI/MySQL_GUI.sql", $dbID);
